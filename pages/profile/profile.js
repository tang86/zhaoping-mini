...
Page({
  ...
  bindMultiPickerChange: function (e) {
    that.updateCity(that.data.multiArray[0][e.detail.value[0]] + '-' + that.data.multiArray[1][e.detail.value[1]]);
    that.setData({
  ...
  intentionPickerSelected: function (option) {
    let This = this;
    let intentionIndex = option.detail.value;
    This.setData({
      intentionIndex: intentionIndex
    });

    This.updateIntentionsName(This.data.intentions[intentionIndex]);
  },
  ...
  updateIntentionsName: function (intentionsName) {
    var This = this;
    var data = {
      intentions_name: intentionsName
    };
    wx.request({
      url: getApp().globalData.postResume.url,
      method: getApp().globalData.postResume.method,
      ...
      success: function (res) {
        if (res.statusCode === 200) {
          let resume = This.data.resume;
          resume.intentions_name = intentionsName;
          This.setData({
            resume: resume
          });
  ...
  getIntentions: function () {
    let This = this;
    wx.request({
      url: getApp().globalData.getIntentions.url,
      method: getApp().globalData.getIntentions.method,
      success: function (res) {
This.setData({
